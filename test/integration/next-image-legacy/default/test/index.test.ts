...
function runTests(mode: 'dev' | 'server') {
  ...
  it('should initially load only two of four images using lazyroot', async () => {
    ...
    try {
      ...
      const query =
        mode === 'server'
          ? getDeploymentId(appDir).getDeploymentIdQuery(true)
          : ''
      expect(imageUrls).toContain(
        `http://localhost:${appPort}/_next/image?url=%2Ftest.jpg&w=828&q=75${query}`
      )
      expect(imageUrls).toContain(
        `http://localhost:${appPort}/_next/image?url=%2Ftest.png&w=828&q=75${query}`
      )
      expect(imageUrls).toContain(
        `http://localhost:${appPort}/test.svg${query}`
      )
      expect(imageUrls).toContain(
        `http://localhost:${appPort}/_next/image?url=%2Ftest.webp&w=828&q=75${query}`
      )
    } finally {
      if (browser) {
        await browser.close()
